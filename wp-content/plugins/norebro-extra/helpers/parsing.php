	...
	class NorExtraParser {
		...
		*/
		static public function VC_typo_to_array( $typo_value ) {
			...
			foreach ( explode( '||', $typo_value ) as $typo_value_row ) {
				...
				if ( $exp[0] == 'font_size' && strlen( NorExtraFilter::string( $exp[1] ) ) > 0 ) {
					$result_arr['font_size'] = NorExtraFilter::string( $exp[1] );
				}
				if ( $exp[0] == 'line_height' && strlen( NorExtraFilter::string( $exp[1] ) ) > 0 ) {
					$result_arr['line_height'] = NorExtraFilter::string( $exp[1] );
				}
				if ( $exp[0] == 'letter_spacing' && strlen( NorExtraFilter::string( $exp[1] ) ) > 0 ) {
					$result_arr['letter_spacing'] = NorExtraFilter::string( $exp[1] );
				}
				if ( $exp[0] == 'weight' ) {
					$result_arr['weight'] = NorExtraFilter::string( $exp[1] );
				}
				if ( $exp[0] == 'italic' ) {
					$result_arr['italic'] = NorExtraFilter::boolean( $exp[1] );
				}
				if ( $exp[0] == 'normal' ) {
					$result_arr['normal'] = NorExtraFilter::boolean( $exp[1] );
				}
				if ( $exp[0] == 'underline' ) {
					$result_arr['underline'] = NorExtraFilter::boolean( $exp[1] );
				}
				if ( $exp[0] == 'uppercase' ) {
					$result_arr['uppercase'] = NorExtraFilter::boolean( $exp[1] );
				}
				if ( $exp[0] == 'use_custom_font' ) {
					$result_arr['use_custom_font'] = NorExtraFilter::boolean( $exp[1] );
				}
				if ( $exp[0] == 'custom_font' && strlen( NorExtraFilter::string( $exp[1] ) ) > 0 ) {
					$result_arr['custom_font'] = NorExtraFilter::string( $exp[1] );
				}
			}
			return $result_arr;
		}
		...
		*/
		static public function VC_typo_to_CSS( $typo_value ) {
			$typo_array = NorExtraParser::VC_typo_to_array( $typo_value );
			$result_string = '';
			$use_font = false;
			$custom_font = false;
		...
		*/
		static public function VC_typo_custom_font( $typo_value ) {
			...
			foreach ( explode( '||', $typo_value ) as $typo_value_row ) {
				$exp = explode( '~', $typo_value_row );
				if ( $exp[0] == 'use_custom_font' )	{
					$use_font = NorExtraFilter::boolean( $exp[1] );
				}
				if ( $exp[0] == 'custom_font' ) {
					$font_key = NorExtraFilter::string( $exp[1] );
				}
			}
